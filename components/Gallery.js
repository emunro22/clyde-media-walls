...
export default function Gallery() {
  ...
  return (
    <section id="gallery" style={{ background: '#080f1c', padding: '100px 24px' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto' }}>

        <div style={{ textAlign: 'center', marginBottom: '64px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '14px', marginBottom: '16px' }}>
            <div style={{ width: '40px', height: '1px', background: CYAN }} />
            <span style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 600, fontSize: '0.78rem', letterSpacing: '0.28em', textTransform: 'uppercase', color: CYAN }}>Recent Projects</span>
            <div style={{ width: '40px', height: '1px', background: CYAN }} />
          </div>
            ...
            <div key={i}
              ...
              <div style={{
                ...
                background: hovered === i
                  ? 'linear-gradient(0deg, rgba(8,15,28,0.92) 0%, rgba(8,15,28,0.3) 100%)'
                  : 'linear-gradient(0deg, rgba(8,15,28,0.7) 0%, transparent 60%)',
                transition: 'all 0.4s ease',
              }} />

              {/* Cyan corner */}
              <div style={{ position: 'absolute', top: 0, right: 0, width: '36px', height: '36px', borderTop: `2px solid rgba(0,200,240,0.6)`, borderRight: `2px solid rgba(0,200,240,0.6)`, transition: 'all 0.3s', opacity: hovered === i ? 1 : 0.4 }} />
              ...
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '24px', transform: hovered === i ? 'translateY(0)' : 'translateY(8px)', transition: 'transform 0.4s ease' }}>
                <div style={{ fontFamily: 'Barlow Condensed, sans-serif', fontSize: '0.7rem', letterSpacing: '0.2em', color: CYAN, textTransform: 'uppercase', marginBottom: '6px' }}>
                  📍 {p.location}
                </div>
                <h3 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '1.5rem', letterSpacing: '0.05em', color: '#e8f4fa', marginBottom: '10px', lineHeight: 1 }}>{p.title}</h3>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', opacity: hovered === i ? 1 : 0, transition: 'opacity 0.3s 0.1s' }}>
                  {p.tags.map(tag => (
                    <span key={tag} style={{ fontFamily: 'Barlow Condensed, sans-serif', fontSize: '0.62rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: CYAN2, background: 'rgba(0,200,240,0.12)', padding: '3px 8px', border: `1px solid rgba(0,200,240,0.2)` }}>{tag}</span>
                  ))}
                </div>
              </div>
            </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '48px' }}>
          <p style={{ fontFamily: 'Barlow, sans-serif', color: 'rgba(232,244,250,0.45)', fontSize: '0.9rem', fontStyle: 'italic' }}>
