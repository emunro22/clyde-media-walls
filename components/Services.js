...
export default function Services() {
  return (
    <section id="services" style={{ background: 'linear-gradient(180deg, #0d1b2e 0%, #0f1f35 100%)', padding: '100px 24px' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '64px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '14px', marginBottom: '16px' }}>
            ...
            <span style={{ fontFamily: 'Barlow Condensed, sans-serif', fontWeight: 600, fontSize: '0.78rem', letterSpacing: '0.28em', textTransform: 'uppercase', color: CYAN }}>What We Offer</span>
            <div style={{ width: '40px', height: '1px', background: CYAN }} />
          </div>
          ...
          <p style={{ fontFamily: 'Barlow, sans-serif', fontSize: '1.05rem', color: 'rgba(232,244,250,0.6)', maxWidth: '540px', margin: '0 auto', lineHeight: 1.7, fontWeight: 300 }}>
            From design through to installation, we deliver complete media wall solutions tailored to your home and vision.
          </p>
        </div>

        {/* Grid */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2px', background: 'rgba(0,200,240,0.1)' }}>
          {services.map((s, i) => (
            <div key={i} style={{
              background: '#0d1b2e', padding: '40px 36px', position: 'relative',
              overflow: 'hidden', transition: 'all 0.3s ease', cursor: 'default',
              borderBottom: '1px solid rgba(0,200,240,0.08)',
            }}
              onMouseEnter={e => { e.currentTarget.style.background = '#112240'; e.currentTarget.querySelector('.service-icon').style.transform = 'scale(1.1) rotate(5deg)' }}
              onMouseLeave={e => { e.currentTarget.style.background = '#0d1b2e'; e.currentTarget.querySelector('.service-icon').style.transform = 'scale(1) rotate(0deg)' }}>

              {/* Cyan corner accent */}
              <div style={{ position: 'absolute', top: 0, left: 0, width: '40px', height: '40px', borderTop: `2px solid rgba(0,200,240,0.4)`, borderLeft: `2px solid rgba(0,200,240,0.4)` }} />
              ...
              <h3 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '1.7rem', letterSpacing: '0.06em', color: '#e8f4fa', marginBottom: '12px', lineHeight: 1 }}>{s.title}</h3>
              <p style={{ fontFamily: 'Barlow, sans-serif', fontSize: '0.92rem', color: 'rgba(232,244,250,0.6)', lineHeight: 1.7, marginBottom: '20px', fontWeight: 300 }}>{s.desc}</p>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {s.tags.map(tag => (
                  <span key={tag} style={{
                    ...
                    color: CYAN, padding: '4px 10px', border: `1px solid rgba(0,200,240,0.3)`,
                  }}>{tag}</span>
                ))}
              </div>
            </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '56px' }}>
          <p style={{ fontFamily: 'Barlow, sans-serif', color: 'rgba(232,244,250,0.5)', marginBottom: '24px', fontSize: '0.95rem', fontStyle: 'italic' }}>Not sure which package suits you? We offer a free, no-obligation design consultation.</p>
          <button onClick={() => document.querySelector('#contact')?.scrollIntoView({ behavior: 'smooth' })}
            ...
            onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 8px 30px rgba(0,200,240,0.4)' }}
